...
- 要約: 訳アリさんがそのテーマについてどう語っているかを400〜600文字で具体的に
...
def extract_json(text: str) -> list:
    ...
    if start == -1 or end == 0:
        raise ValueError(f"JSONが見つかりませんでした。レスポンス:\n{text[:500]}")
...
def main():
    ...
    print(f"Gemini API にリクエスト中... ({len(sample)}件のQ&Aを送信)")
    ...
    print(f"完了: {len(summaries)}テーマを生成 → {SUMMARIES_FILE}")
